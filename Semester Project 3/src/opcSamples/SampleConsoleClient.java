...
public class SampleConsoleClient {
  ...
  protected boolean discover() throws URISyntaxException, ServerListException {
    ...
    discoveryUrls = discoverServer(client.getUri());
  ...
  protected void readHistory(NodeId nodeId) throws ServiceException, StatusException, AddressSpaceException {
    ...
    if (node instanceof UaVariable) {
      ...
      if (!variable.getUserAccessLevel().contains(AccessLevel.HistoryRead)) {
        ...
        switch (action) {
          ...
          default:
            ...
            AggregateConfiguration aggregateConfiguration =
                new AggregateConfiguration(false, true, new UnsignedByte(100), new UnsignedByte(100), false);
      ...
      try {
        ...
        if (events != null) {
          ...
          for (int i = 0; i < events.length; i++) {
            println("Event " + (i + 1) + " = " + eventFieldsToString(eventFieldNames, events[i].getEventFields()));
          }
        }
      } catch (Exception e) {
        printException(e);
